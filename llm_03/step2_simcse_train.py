...
dev_df_ids = dev_df.url.unique().tolist()
...
def in_batch_loss(feature_topic, feature_content):
    ...
    cosine_sim = torch.matmul(feature_topic, feature_content.T)
    ...
    cosine_sim = cosine_sim - torch.diag(margin_diag).to('cuda')
    ...
    labels = torch.reshape(labels, shape=[-1]).to('cuda')
